...
try:
    ...
    assert False, "You must first install pyobjc-core and pyobjc: https://GUIautomate.readthedocs.io/en/latest/install.html"
import AppKit
...
if sys.platform !=  'darwin':
    raise Exception('The GUIautomate_osx module should only be loaded on an OS X system.')
...
def _normalKeyEvent(key, upDown):
    ...
    try:
        if GUIautomate.isShiftCharacter(key):
            ...
            time.sleep(GUIautomate.DARWIN_CATCH_UP_TIME)
...
def _dragTo(x, y, button):
    if button == LEFT:
        ...
        assert False, "button argument not in ('left', 'middle', 'right')"
    time.sleep(GUIautomate.DARWIN_CATCH_UP_TIME) # needed to allow OS time to catch up.
